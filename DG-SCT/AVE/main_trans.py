...
from nets.net_trans import PromptedMMIL_Net
...
def main():
	...
	if args.wandb:
		wandb.init(config=args, project="dg_sct_global",name=args.model_name)

	if args.model == 'Prompted_MMIL_Net':
		model = PromptedMMIL_Net(args).to('cuda')
		# model.double()
	else:
	...
	if args.mode == 'train':
		...
		if args.init_best:
			print('Loading pre-trained model for resuming training...')
			checkpoint = torch.load("/workspace/DG-SCT/DG-SCT/AVE/models/checkpoint_swin/best_80.72.pt")
		...
		# print('####### Prompt tokens params: %0.4f  #######'%(prompt_tokens_params*100/total_params))
		# print('####### Temporal attn params: %0.4f  #######'%(temporal_attn_params*100/total_params))
		...
		if args.init_best:
			print('Loading pre-trained optimizer and scheduler...')
		...
		for epoch in range(1, args.epochs + 1):
			...
			count +=1
			wandb_log = {"val-accuracy":F_event}
			if  F_event >= best_F:
				count = 0
				best_F = F_event
				print('#################### save model #####################')
				...
				}, args.model_save_dir + args.checkpoint + "_%0.2f.pt"%(best_F))
				wandb_log["val-best"]=F_event
		...
		model.load_state_dict(torch.load("/workspace/data/DG_SCT/baidu/best_82.18.pt"), strict=False)
