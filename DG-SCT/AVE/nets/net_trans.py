...
from nets.prompted_models import PromptedSwinV2, PromptedHTSAT
...
class MMIL_Net(nn.Module):
	...
	def __init__(self, opt):
		...
		if opt.backbone_type == "esc-50":
			...
			esc_config.checkpoint_path = "/workspace/data/AVMOE"
...
class PromptedMMIL_Net(MMIL_Net):
	def __init__(self, opt):
		super().__init__(opt)
		...
		self.swin = PromptedSwinV2(self.prompt_config,
									img_size=192,
									patch_size=(4,4),
									num_classes=21841,
									embed_dim=192,
		...
		checkpoint_path="/workspace/data/DG_SCT/swinv2_large_patch4_window12_192_22k.pth"
		...
		# self.htsat = PromptedHTSAT(
		# 	num_prompts=prompt_config.NUM_PROMPTS,
		# 	prompt_dim=96,
		# 	spec_size=esc_config.htsat_spec_size,
		# 	patch_size=esc_config.htsat_patch_size,
		# 	in_chans=1,
		...
		# tmp = torch.load(checkpoint_path, map_location='cpu')
		# tmp = {k[10:]:v for k, v in tmp['state_dict'].items()}
		# self.htsat.load_state_dict(tmp, strict=False)
		...
		## ------------> for swin and htsat 
		for idx_layer, (my_blk, my_blk_a) in enumerate(zip(self.swin.layers, self.htsat.layers)):
			conv_dim_tmp = (my_blk.input_resolution[0]*my_blk.input_resolution[1]+self.prompt_config.NUM_PROMPTS)
			conv_dim_tmp_a = (my_blk_a.input_resolution[0]*my_blk_a.input_resolution[1])
			if not isinstance(my_blk.downsample, nn.Identity):
				down_in_dim.append(my_blk.downsample.reduction.in_features)
				down_out_dim.append(my_blk.downsample.reduction.out_features)
	...
	def forward_swin(self, audio, vis, mixup_lambda, rand_train_idx=12, stage='eval'):
		...
		bs = vis.size(0)
		vis = rearrange(vis, 'b t c w h -> (b t) c w h')
		f_v = self.swin.incorporate_prompt(vis)
		...
		idx_layer = 0
		out_idx_layer = 0
		for _, (my_blk, htsat_blk, deep_prompt_embd) in enumerate(zip(self.swin.layers, self.htsat.layers, self.swin.deep_prompt_tokens)) :
						...
						if deep_prompt_embd.shape[0] != num_blocks:
							...
							for i in range(num_blocks):
								if i != 0:
									...
									f_v = torch.cat(
										(prompt_emb, f_v[:, self.prompt_config.NUM_PROMPTS:, :]),
										dim=1
									)
						else:
							# other layers
	...
	def forward(self, audio, vis, mixup_lambda=None, rand_train_idx=12, stage='eval'):
		return self.forward_swin(audio, vis, mixup_lambda, rand_train_idx, stage)
