...
class AVE_dataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		for vis_idx in range(10):
			...
			tmp_img = torchvision.io.read_image(self.opt.video_folder+'/'+file_name+'/'+ str("{:08d}".format(tmp_idx))+ '.jpg')/255
			tmp_img = self.my_normalize(tmp_img)
			total_img.append(tmp_img)
		total_img = torch.stack(total_img)
		### <---

		wave = np.load(os.path.join(self.opt.root_path, '/workspace/data/DG_SCT/baidu/wave/{}.npy'.format(file_name)), allow_pickle=True)
